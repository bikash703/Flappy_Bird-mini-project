...
import sys
from numpy import geomspace # We wil use sys.exit to exit the program
import pygame 
...
def MainGame():
    ...
    #create 2 pipes for the blitting on the screen
    ...
    playerFlapped = False #It is true only the bird is flapping
    ...
    while True:
        ...
        crashTest = isCollide(playerx, playery, upperPipes, lowerPipes) # This function will return true if the palyer is crashed
...
def getRandomPipe():
       """
       Generate position of two pipes(one the bottom straight and one top rotated ) for blitting on the screen
       """
       pipeHeight = GAME_SPRITES['pipe'][0].get_height()
